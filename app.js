...
/* ==========================================================
   V18 — Force-visible Kana entry
   ========================================================== */
function v18EnsureKanaButton(){
  if(currentTab!=='vocab')return;
  const content=document.getElementById('lesson-content')||document.querySelector('.lesson-content')||document.getElementById('app');
  if(!content)return;

  // If V17 renderer is active, this selector exists.
  if(document.querySelector('.v17-vocab-switch'))return;

  // Fallback: if an older render appeared for any reason, inject a direct entry.
  const oldSwitch=document.querySelector('.vocab-mode-switch');
  if(oldSwitch && !document.getElementById('v18-kana-force-btn')){
    const b=document.createElement('button');
    b.id='v18-kana-force-btn';
    b.className='active kana';
    b.innerHTML='🌱 Học từ mới • Kana';
    b.onclick=()=>{vocabStudyMode='kana-new';renderLessonContent();};
    oldSwitch.prepend(b);
  }
}
const _v18RenderLessonContent=renderLessonContent;
renderLessonContent=function(){
  const r=_v18RenderLessonContent();
  setTimeout(v18EnsureKanaButton,0);
  return r;
};

/* Opening a lesson's vocab tab defaults to Kana-first in V18. */
const _v18OpenLesson=openLesson;
openLesson=function(id,tab){
  if(tab==='vocab')vocabStudyMode='kana-new';
  const r=_v18OpenLesson(id,tab);
  if(tab==='vocab'){
    vocabStudyMode='kana-new';
    setTimeout(()=>{try{renderLessonContent()}catch{}},0);
  }
  return r;
};
